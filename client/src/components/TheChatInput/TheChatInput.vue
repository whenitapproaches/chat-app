		...
		<div class="columns is-vcentered is-fullwidth is-gapless">
			...
			<div class="column is-narrow">
				<div class="toolbar emoji mr-4">
					...
					<TheChatInputEmojis @select-emoji="selectEmoji" v-show="isEmojiListOpened" />
				</div>
			</div>
			<div class="column is-narrow">
...
export default {
	...
	setup() {
		...
		const sendMessage = (event, media = "", file) => {
			...
			const { _id: currentUserId } = currentUser.value

			const receiverUsername = currentPartnerUsername.value

			const conversation = find(
				conversations.value,
				(o) => o.user.username === receiverUsername
			)
